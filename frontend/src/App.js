...
function MainApp() {
  ...
  const handleSaveContext = async () => {
    ...
    try {
      ...
      await axios.put(`/content/${previewContent.id}/context`, {
      ...
      await axios.put(`/content/${previewContent.id}/title`, {
      ...
      if (response.data.created && response.data.created.length > 0) {
        ...
        for (let i = 0; i < response.data.created.length; i++) {
          ...
          try {
            ...
            if (customTitle) {
              const titleURL = `${API.replace('/api', '')}/content/${createdItem.id}/title`; // CORRECTION: enlever /api du domaine !
            ...
            if (customContext) {
              const contextURL = `${API.replace('/api', '')}/content/${createdItem.id}/context`; // CORRECTION: enlever /api du domaine !
