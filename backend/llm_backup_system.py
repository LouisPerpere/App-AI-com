...
class LLMBackupSystem:
    ...
    async def analyze_website_content(
        ...
        prompt = f"""
        Analyse ce site web et réponds UNIQUEMENT par un JSON structuré avec les clés suivantes:
        {{
            "analysis_summary": "Un résumé détaillé de l'entreprise et de ses activités (300-400 mots incluant QUI ils sont, CE QU'ILS FONT, COMMENT ils le font, leurs spécialités, valeurs, expérience)",
            "key_topics": ["topic1", "topic2", "topic3", "topic4", "topic5", "topic6", "topic7"],
            "brand_tone": "le ton de communication (professionnel/décontracté/premium/accessible/expert)",
            "target_audience": "description précise du public cible avec démographie, besoins, motivations",
            "main_services": ["service détaillé 1", "service détaillé 2", "service détaillé 3", "service détaillé 4"],
            "content_suggestions": ["suggestion 1 basée sur services", "suggestion 2 basée sur expertise", "suggestion 3 basée sur produits", "suggestion 4 basée sur valeurs", "suggestion 5 basée sur audience", "suggestion 6 créative"]
        }}
        ...
        Contenu principal: {content_data.get('text_content', 'Non disponible')[:3000]}

        INSTRUCTIONS:
        - Sois très détaillé dans analysis_summary (300-400 mots)
        - Identifie tous les produits/services avec leurs spécificités
        - Extrais les témoignages, réalisations, différenciateurs
        - Suggère du contenu exploitable pour les réseaux sociaux
        - Utilise les termes exacts trouvés sur le site
