...
async def get_pending_content_mongo(
    offset: int = 0,
    limit: int = 24,
    user_id: str = Depends(get_current_user_id_robust)  # Use robust authentication
):
...
def generate_thumbnail_background(file_id: str, unique_filename: str, file_path: str, content_type: str):
    """Background task for thumbnail generation with immediate parameter binding"""
    try:
        ...
        if not os.path.exists(thumb_path):
            print(f"❌ Thumbnail file was not created: {thumb_path}")
            return
        ...
        if mongo_url:
            ...
            if result.modified_count == 1:
                ...
                print(f"❌ Failed to update MongoDB for {unique_filename} (file_id: {file_id})")
