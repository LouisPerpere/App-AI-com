                          ...
                          <div className="mt-6">
                            <div className="flex items-center justify-between mb-4">
                              <h4 className="text-lg font-semibold text-gray-900 flex items-center">
                                ...
                                Carrousel : {carouselFiles.length} image{carouselFiles.length > 1 ? 's' : ''} sélectionnée{carouselFiles.length > 1 ? 's' : ''}
                              </h4>
                              <Button
                                onClick={() => {
                                  setCarouselFiles([]);
                            ...
                            {/* Stack preview - first image visible, others behind */}
                            <div className="mb-6">
                                  ...
                                  return (
                                    <div
                                      ...
                                      style={{
                                        ...
                                        zIndex: carouselFiles.length - index,
                                    ...
                                    >
                                      ...
                                      {index === 0 && (
                                        <div className="absolute top-2 right-2 bg-pink-600 text-white text-xs px-2 py-1 rounded-full font-medium">
                                          {carouselFiles.length} photo{carouselFiles.length > 1 ? 's' : ''}
                                        </div>
                                      )}
                                    </div>
                                  );
                            ...
                            {/* Common title and context for carousel */}
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                              <div>
                                <label className="block text-sm font-medium text-gray-700 mb-2">
                                  Titre commun du carrousel *
                                </label>
                                <input
                                  type="text"
                                  value={carouselTitle}
                                  onChange={(e) => setCarouselTitle(e.target.value)}
                                  placeholder="Ex: Photos de vacances été 2024"
                              ...
                              <div>
                                <label className="block text-sm font-medium text-gray-700 mb-2">
                                  Contexte commun (optionnel)
                                </label>
                                <input
                                  type="text"
                                  value={carouselContext}
                                  onChange={(e) => setCarouselContext(e.target.value)}
                                  placeholder="Description du carrousel..."
                                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-pink-500 focus:border-transparent"
                                />
                              </div>
                            </div>
                            
                            {/* Upload carousel button */}
                            <div className="text-center">
                              <Button
                                onClick={handleCarouselUpload}
                                disabled={isUploadingToMonth || !carouselTitle.trim() || !globalUploadMonth}
                                className="bg-gradient-to-r from-pink-500 to-pink-600 hover:from-pink-600 hover:to-pink-700 text-white px-8 py-3 text-lg font-medium"
                              >
                                {isUploadingToMonth ? (
                                  <>
                                    <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin mr-2"></div>
                                    Upload carrousel en cours...
                                  </>
                                ) : (
                                  <>
                                    <ImageIcon className="w-5 h-5 mr-2" />
                                    Uploader le carrousel ({carouselFiles.length} images)
                                  </>
                                )}
                              </Button>
                              
                              {(!globalUploadMonth || !carouselTitle.trim()) && (
                                <p className="text-red-600 text-sm mt-2">
                                  {!globalUploadMonth && '⚠️ Sélectionnez un mois de destination'}
                                  {!globalUploadMonth && !carouselTitle.trim() && ' • '}
                                  {!carouselTitle.trim() && '⚠️ Saisissez un titre'}
                                </p>
                              )}
                            </div>
                          </div>
